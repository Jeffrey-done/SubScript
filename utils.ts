...
export const startOfLocalDay = (value: Date | string): Date => {
  const date = typeof value === 'string' ? parseDateOnly(value) : new Date(value);
  return new Date(date.getFullYear(), date.getMonth(), date.getDate());
};

const addCycle = (date: Date, cycle: CycleType): Date => {
...
export const advanceBillingDate = (startDateStr: string, cycle: CycleType): Date => {
