...
import { calculateStats, formatCurrency, getDaysInMonth, countStandardRestDays, getPaydayCountdown, parseDateOnly } from '../utils';
...
const Dashboard: React.FC<DashboardProps> = ({ subscriptions, budget, onUpdateBudget, restDays, onToggleRestDay, onOpenAI, transactions }) => {
  ...
  const mergedChartData = useMemo(() => {
    ...
    const monthlyExpenses = transactions.filter(t => 
        t.type === 'expense' && t.source !== 'renewal' && t.date.startsWith(viewMonthPrefix)
    );
  ...
  const paydayCountdown = useMemo(() => {
    if (budget.payday <= 0) return null;
    return getPaydayCountdown(budget.payday);
  }, [budget.payday]);
  ...
  const calculateSalaryForMonth = (date: Date) => {
    ...
    const base = budget.baseSalary > 0 ? budget.baseSalary : 0;
  ...
  const variableExpenses = useMemo(() => {
    return transactions
        .filter(t => t.type === 'expense' && t.source !== 'renewal' && t.date.startsWith(currentRealMonthPrefix))
  ...
  const calendarData = useMemo(() => {
    ...
    subscriptions.forEach(sub => {
        const start = parseDateOnly(sub.startDate);
  ...
  const expectedPayDate = budget.payday > 0
      ? new Date(viewMonthStats.year, viewMonthStats.month - 1, budget.payday)
      : null;
  if (expectedPayDate && budget.salaryDelay === 1) {
      expectedPayDate.setMonth(expectedPayDate.getMonth() + 1);
  }
  const expectedPayDateStr = expectedPayDate
      ? `${expectedPayDate.getMonth() + 1}月${expectedPayDate.getDate()}日`
      : '未设置';
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between group/row">
                             ...
                             {editingTarget === 'payday' ? (
                                ...
                                <span className="font-medium text-main cursor-pointer hover:text-primary transition-colors flex items-center gap-1" onClick={() => startEditing('payday', budget.payday > 0 ? budget.payday : 15)}>
                                    每月 {budget.payday > 0 ? budget.payday : 15} 号 <Edit2IconMini />
                                </span>
                             )}
                        </div>
                    ...
                    <div className="flex flex-col text-right border-l border-border pl-4">
                        ...
                        <div className="mt-2">
                            ...
                            <p className={`text-2xl font-bold ${disposableIncome >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                {budget.baseSalary > 0 ? formatCurrency(disposableIncome) : '未设置'}
                            </p>
