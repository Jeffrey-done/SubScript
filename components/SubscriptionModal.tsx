...
const SubscriptionModal: React.FC<Props> = ({ isOpen, onClose, onSave, initialData, aiConfig }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        setStartDate(formatDateOnly(initialData.startDate));
  ...
  const handleGenerateImage = async () => {
      ...
      try {
          ...
          console.error(e);
          setGenError(e.message || '生成失败，请检查设置中的 API 密钥');
      } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-label="订阅弹窗">
      <div className="bg-surface border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-5 border-b border-border bg-background/50 sticky top-0 backdrop-blur-md z-10">
          ...
          <button type="button" onClick={onClose} aria-label="关闭订阅弹窗" className="text-muted hover:text-main transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-muted mb-1">价格 (元)</label>
              <input
                type="number"
                required
                min="0"
