...
const TransactionModal: React.FC<Props> = ({ isOpen, onClose, onSave, initialData, aiConfig }) => {
  ...
  useEffect(() => {
      ...
      if (initialData) {
        ...
        setDate(formatDateOnly(initialData.date));
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          if (result.amount > 0) {
              ...
              setAiError('识别不完整（未找到金额）。请在下方补充缺失信息。');
          }
  ...
  const handleTextSubmit = async () => {
      ...
      try {
          ...
          if (result.amount > 0) {
              ...
              setAiError("仍然无法确定金额，请明确说明（例如：'金额是 0 元'）");
          }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-label="记账弹窗">
      <div className="bg-surface border border-border rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border bg-background/50">
           ...
           <button type="button" onClick={onClose} aria-label="关闭记账弹窗" className="text-muted hover:text-main transition-colors p-1">
              <X className="w-5 h-5" />
           </button>
        </div>
