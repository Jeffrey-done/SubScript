...
import { CalendarClock, Trash2, Edit2, CreditCard } from 'lucide-react';
...
const SubscriptionList: React.FC<Props> = ({ subscriptions, onDelete, onEdit, onRenew, isLoading = false }) => {
  ...
  if (subscriptions.length === 0) {
    return (
      <div className="text-center py-20 bg-surface rounded-2xl border border-border border-dashed animate-in fade-in zoom-in duration-300">
        ...
        <h3 className="text-lg font-medium text-main">暂无订阅</h3>
        <p className="text-muted mt-1">点击右上角按钮添加你的第一个订阅。</p>
      </div>
    );
  }
            ...
            <div className="flex items-center justify-between mt-4 pl-3 pt-4 border-t border-border">
                ...
                <div className="flex gap-2 relative z-20 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <button
                        ...
                        aria-label={`续费 ${sub.name}`}
                        className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-muted hover:text-emerald-500 dark:hover:text-emerald-400 transition-all active:scale-90 active:bg-slate-200 dark:active:bg-slate-600"
                        title="确认续费/记账"
                    >
