...
const AIAnalysisModal: React.FC<Props> = ({ isOpen, onClose, subscriptions, budget, aiConfig, transactions }) => {
  ...
  const renderedContent = DOMPurify.sanitize(marked.parse(content, { async: false }) as string);

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-label="AI分析弹窗">
      <div className="bg-surface border border-border rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[85vh]">
        ...
        <div className="flex justify-between items-center p-5 border-b border-border bg-background/50 sticky top-0 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={handleClose} aria-label="关闭 AI 分析弹窗" className="text-muted hover:text-main transition-colors p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full">
                <X className="w-5 h-5" />
            </button>
          </div>
        </div>
